+++
title = "A Malicious SIM Card Can Run Attacker Code Inside the Modems Behind Cellular IoT Devices"
date = 2026-08-11
description = "Researchers discovered a malicious SIM card that can control devices with malicious software by running commands on the cellular module inside them. The exploit is potentially used to take control of IoT devices such as electric vehicle chargers, industrial routers, and car telematics units."
tags = ["unread"]
extra.source_url = "https://thehackernews.com/2026/08/a-malicious-sim-card-can-run-attacker.html"
+++

A malicious SIM card can order the device it sits in to run commands of the attacker's choosing. On the cellular modules built into electric-vehicle chargers, industrial routers, and car telematics units, that is enough to take the whole device over. Researchers at the University of Birmingham and the security firm Fuzzware tested 26 phones and cellular modules for the capability, found it
